<?php
	...
	if(isset($_GET['ok']))
	{
		$sql_parse = OCIParse($connect, "DELETE FROM devices WHERE dev_name = '".trim($_GET['dev_name'])."'");
		OCIExecute($sql_parse, OCI_DEFAULT);
		OCICommit($connect);
		$_SESSION['error']='Устройство удалено';	
	}
?>
<html>
...
<head>
	...
	<title>Удаление устройства</title>
...
<body>
	<div class="mainbody">
		...
		<div class='content'>
			...
			<form action="delete_devices.php" method="GET" class="authform">
				Выберите название удаляемого устройства<br>
				<select class='roles' name='dev_name'>
					<?php
						$sql_parse = OCIParse($connect , "SELECT * FROM devices");
						OCIExecute($sql_parse, OCI_DEFAULT);
						while(OCIFetch($sql_parse))
						{
							echo "<option value=".OCIResult($sql_parse, 'DEV_NAME').">".OCIResult($sql_parse, 'DEV_NAME')."</option>";
						}
					?>
				</select><br><br>
