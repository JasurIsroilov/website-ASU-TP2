<?php
	...
	if(isset($_GET['ok']))
	{
		$sel_parse = OCIParse($connect, "SELECT * FROM equipment WHERE
								equ_name = '".$_GET['old_equ_name']."'");
		OCIExecute($sel_parse, OCI_DEFAULT);
		OCIFetch($sel_parse);
		if(!trim($_GET['equ_name']))
		{
			$equ_name = OCIResult($sel_parse, 'EQU_NAME');
		}
		else
		{
			$equ_name = trim($_GET['equ_name']);
		}
		
		if(!trim($_GET['equ_type']))
		{
			$equ_type = OCIResult($sel_parse, 'EQU_TYPE');
		}
		else
		{
			$equ_type = trim($_GET['equ_type']);
		}
		
		$upd_parse = OCIParse($connect, "UPDATE equipment 
					SET equ_name = '".$equ_name."'
					, equ_type = '".$equ_type."'
					WHERE equ_name = '".$_GET['old_equ_name']."'");
...
<body>
	<div class="mainbody">
		...
		<div class='content'>
			...
			<form action="edit_equipment.php" method="GET" class="authform">
				Выберите название изменяемого оборудования<br>
				<select class='roles' name='old_equ_name'>
					<?php
