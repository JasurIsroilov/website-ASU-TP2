<?php
	...
	if(isset($_GET['ok']))
	{
		$sql_parse = OCIParse($connect, "DELETE FROM documents WHERE doc_name = '".trim($_GET['doc_name'])."'");
		OCIExecute($sql_parse, OCI_DEFAULT);
		OCICommit($connect);
		$_SESSION['error']='Документ удален';	
	}
?>
<html>
...
<body>
	<div class="mainbody">
		...
		<div class='content'>
			...
			<form action="delete_documents.php" method="GET" class="authform">
				Выберите название удаляемого устройства<br>
				<select class='roles' name='doc_name'>
					<?php
						$sql_parse = OCIParse($connect , "SELECT * FROM documents");
						OCIExecute($sql_parse, OCI_DEFAULT);
						while(OCIFetch($sql_parse))
						{
							echo "<option value=".OCIResult($sql_parse, 'DOC_NAME').">".OCIResult($sql_parse, 'DOC_NAME')."</option>";
						}
					?>
				</select><br><br>
